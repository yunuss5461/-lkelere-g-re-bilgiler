...
// Kartları gösteren fonksiyon
function showCards(data) {
    const cardsContainer = document.getElementById('cards-container');
    cardsContainer.innerHTML = '';

    data.forEach(country => {
        const population = country.population ? country.population.toLocaleString() : 'Bilgi Yok';
        const area = country.area ? country.area.toLocaleString() : 'Bilgi Yok';

        const cardHtml = `
            <div class="card">
                <img src="${country.flag}" alt="Flag of ${country.name}" class="card-img">
                <div class="card-body">
                    <h3 class="card-title">${country.name}</h3>
                    <p class="card-text">Başkent: ${country.capital}</p>
                    <p class="card-text">Nüfus: ${population}</p>
                    <p class="card-text">Bölge: ${country.region}</p>
                    <p class="card-text">Alan: ${area} km²</p>
                    <p class="card-text">Diller: ${country.languages.join(', ')}</p>
                </div>
            </div>
        `;
...
document.getElementById('show-card').addEventListener('click', () => showCards(countries_data));
document.getElementById('sort-alpha').addEventListener('click', () => showCards(sortByAlpha()));
document.getElementById('sort-population').addEventListener('click', () => showCards(sortByPopulation()));
